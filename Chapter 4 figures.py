...
#figure 7
#runs a simple HB momentum scheme and plots the corrresponding 1\(1-lambda) value for comparison
...
# code reproduced from https://www.tcbegley.com/blog/posts/mcmc-part-1
#figure 8
def metropolis(target, initial, proposal, iterations=100_000):
    #the metropolis hasting algorithm as set up in the reference above
...
#discretisation systems as seen in paper refernces in hmc section of thesis by Neal
#reproduced from Neal source
#figure 10
#all discretisation schemes code algorithms can be found in referenced source
def euler(q, p, eps, n):
    Q = [q]
    P = [p]
    for _ in range(n):
...
settings = [
    ...
    (r'Leapfrog, $\epsilon=0.3$', leapfrog_harm, 0.3),#showing multiple leapfrog values to should how it moves 
    (r'Leapfrog, $\epsilon=1.2$', leapfrog_harm, 1.2),#around the trajectory
]
...
# Figure 12 
#reproduced from Neal paper, example of code can be seen at https://github.com/jnhwkim/HMC/blob/master/hmc.m
# this is an adaptation of it
#all discretisation schemes code algorithms can be found online in referenced sources
...
Sigma=np.array([[1,0.95],[0.95,1]]); Prec=np.linalg.inv(Sigma) # the choice of covariance matrix as stated in reference
