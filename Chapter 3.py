...
#firstly we define all the basic functiosn to be inline iwth the paper exerts we wish to recreat:
...
#momentum function with iterative steps as defined in kovachki pape

    n_steps = int(np.floor(T / h)) 
    ts = np.arange(n_steps + 1) * h

    us = np.zeros((n_steps + 1, len(u0)), dtype=float)
    us[0] = u0 #using initial starting value foe itertaive scheme 
...
def ham_traj(ts: np.ndarray,u0: np.ndarray,Q: np.ndarray,lam: float,a: float,h: float,
    ...
    for j, qj in enumerate(q):
        ...
        if disc > 0:
            ...
            # u_j(t) = e^{real t}(c1 cos(imag t) + c2 sin(imag t))
            c1 = u0[j]
...
#recreation of plots from su boyd and candes paper ended up not using in paper
r = 1.0
x0 = 1.0
s = 1e-4
...
def ode_rhs(t, z):
    x, v = z
    damping = r / t
    return [v, -damping * v - grad(x)]

eps = 1e-6 #not from 0 as we get issue because of singularity
z_eps = [
    x0 - x0 * eps**2 / (2 * (r + 1)),
    -x0 * eps / (r + 1)
]

t_eval = np.linspace(eps, 50, 5000)
sol = solve_ivp(
    ode_rhs,
    (eps, 50),
    z_eps,
...
for k in range(1, K + 1):
    x[k] = y[k - 1] - s * grad(y[k - 1])
    momentum = (k - 1) / (k + r - 1)
    y[k] = x[k] + momentum * (x[k] - x[k - 1])

k_vals = np.arange(K + 1)
scaled_scheme_error = s * k_vals**2 * f(x)

fig, axes = plt.subplots(1, 2, figsize=(12, 4))

axes[0].plot(t, scaled_ode_error)
axes[0].set_xlabel(r"$t$")
axes[0].set_ylabel(r"$t^2(f - f^*)$")
axes[0].set_title(r"(a) Scaled errors $t^2(f(X(t))-f^*)$")
axes[0].set_xlim([0, 50])
axes[0].set_ylim([0, 16])

axes[1].plot(k_vals, scaled_scheme_error)
axes[1].set_xlabel("iterations")
axes[1].set_ylabel(r"$sk^2(f - f^*)$")
axes[1].set_title(r"(b) Scaled errors $sk^2(f(x_k)-f^*)$")
...
#attempt to make plotting easier
plt.rcParams.update({
    'figure.dpi': 140,
    'savefig.dpi': 300,
    'font.size': 9,
...
COL = { #researched specific colours f0r consistency›
    'ode': '#222222',
    'hb': '#0173B2',
    'nag': '#DE8F05',
    'gd': '#666666',
